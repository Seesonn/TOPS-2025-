...
const AttendanceCard = ({ name, organization, ticketType = "standard" }) => {
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 group border ${colors.accent} overflow-hidden`}
    >
      {/*  Top Right */}
      <div
        className={`absolute -top-12 -right-12 w-32 h-32 rounded-full ${colors.curve} opacity-10 group-hover:opacity-20 transition-opacity duration-500`}
      ></div>

      {/*  Bottom Left */}
      <div
        className={`absolute -bottom-8 -left-8 w-24 h-24 rounded-full ${colors.curve} opacity-15 group-hover:opacity-25 transition-opacity duration-500`}
      ></div>
